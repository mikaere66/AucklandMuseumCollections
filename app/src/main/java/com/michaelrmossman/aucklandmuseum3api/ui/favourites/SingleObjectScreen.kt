...
import com.michaelrmossman.aucklandmuseum3api.ui.objects.ObjectDetailsCard
import com.michaelrmossman.aucklandmuseum3api.util.titleText

/* Will download a single OpacObject, upon
   clicking favourite MediaType.Object */
@Composable
fun SingleObjectScreen(
    ...
    LaunchedEffect(key1 = Unit) {
        if (
            viewState.favObject != null
            &&
            (viewState.favObject?.opacObjectId.toString()
            !=
            favourite.itemId)
        ) {
    ...
    LaunchedEffect(key1 = downloadedReqd) {
        if (downloadedReqd) {
            viewModel.getObject(favourite.itemId)
        }
    }
...
//                is FavesUiState.EmptyList -> EmptySearchScreen(
//                    modifier = Modifier.fillMaxSize(),
...
//                )
                ...
                is FavesUiState.Success -> {
                    viewState.favObject?.let { `object` ->
                        val titlesFlattened = `object`.titleText()
                        ObjectDetailsCard(
