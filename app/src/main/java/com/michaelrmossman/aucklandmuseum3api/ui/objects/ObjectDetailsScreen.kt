...
fun ObjectDetailsScreen(
    ...
    if (showBottomSheet) {
        ...
        ToggleFaveBottomSheet(
            itemId = currentItem.opacObjectId.toString(),
    ...
    LaunchedEffect(key1 = pagerState) {
        ...
        snapshotFlow { pagerState.currentPage }.collect { page ->
            ...
            if (viewState.resultState == SearchUiState.Error) {
                objectsViewModel.resetErrorState()
            }
        }
    }
    ...
    Scaffold(
        ...
        val content: (@Composable (OpacObject) -> Unit) = { result ->
            val titlesFlattened = result.titleText()
            ObjectDetailsCard(
                contentPadding = contentPadding,
                /* Modifier used by all [OpacObject]s. Padding
